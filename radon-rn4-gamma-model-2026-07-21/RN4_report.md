...
**Методы:** стационарная камерная модель Jacobi с раздельными свободной и присоединённой фракциями и нуклид-специфичным осаждением по камерным измерениям Leonard (1994); кратность воздухообмена измерена по динамике набора радона после выключения вентиляции (0.01–0.07 ч⁻¹), скорость присоединения к аэрозолю задана по нормативному уровню запылённости (X = 25–50 ч⁻¹), коэффициент равновесия — выход модели (F = 0.60–0.71); перенос — прямое интегрирование ядра точечного источника по полному набору гамма-линий LNHB/DDEP (110 линий E ≥ 0.2 МэВ, ≥ 99 % гамма-энергии ДПР) с усреднением поля по объёму кристалла; комптоновское рассеяние — модель полости с дозовым альбедо бетона по формуле Чилтона–Хаддлстона; учтены экранирование стальной столешницей и осаждение на её нижнюю плоскость.

**Вывод:** базовая оценка приращения — **0.67–0.71 нЗв/ч на 100 Бк/м³** (центральное значение ≈ 0.69); структура вкладов: воздух ~65–75 % (при высоком равновесии), поверхности помещения и подставка — остальное. Целевая величина замера с учётом калибровочной погрешности прибора ±20 % образует полосы 0.87–1.31 (межсуточный контраст) и 0.55–1.13 (внутрисуточные оценки): базовая оценка модели **совместима с внутрисуточным замером** (×1.00 к его центру 0.69) и отстоит от нижнего края межсуточной полосы в ×1.26. Кандидаты остатка к межсуточному центру — осаждение на корпус детектора и энергетический отклик прибора по спектру ДПР.
...
Производящий член содержит постоянную распада *дочернего* нуклида λᵢ: Cᵢ₋₁ — активность родителя — равна скорости рождения *атомов* дочернего, а умножение на λᵢ переводит атомный баланс в баланс активностей.
...
Основная модель — полная, с раздельными *свободной* и *присоединённой* фракциями каждого ДПР: присоединение к аэрозолю со скоростью X, отрыв ²¹⁴Pb при α-распаде присоединённого ²¹⁸Po (p = 0.83[7]), нуклид- и материал-специфичное осаждение свободной фракции (Табл. 1), диффузионное осаждение присоединённой (0.1 м/ч[2]) и вертикальный член гравитационного оседания аэрозоля ±vₛ (vₛ = 0.012 м/ч для AMD ≈ 200 нм; пол/подставка +, потолок −). Коэффициент равновесия при этом — **выход** модели:
...
**Численное интегрирование.** Объём — сетка 5 см послойно; сингулярность ядра при r → 0 снимается аналитически: окрестность радиусом 15 см вокруг детектора интегрируется как равномерный шар, (1 − e^(−μR))/μ. Грани — сетка 5 см; расстояние r = √(u² + v² + d²), перпендикуляры d: 1.00 м до ближней стены, 4.00 м до дальней, по 5.50 м до торцевых, 1.00 м до пола, 1.30 м до потолка (минимальное r = 1.00 м, сингулярности нет; осевшая активность — изотропный излучатель на единицу площади, косинусные множители не вводятся). Пофасадные факторы при 0.609 МэВ: пол 0.593, потолок 0.488, ближняя стена 0.417, дальняя 0.081, торцевые по 0.025 — ближняя стена даёт ~76 % вклада стен; в пределе бесконечной плоскости зависимость от расстояния лишь логарифмическая, (S_A/2)·E₁(μd), отсюда слабая чувствительность итога к зазору (10 см → 0.42, 1.0 м → 0.39, центр → 0.40 нЗв/ч на 100 Бк/м³). Подставка — сетка 5 мм, поле **усредняется по объёму кристалла** (сетка 4 мм, ~800 точек внутри цилиндра): в ближней зоне точечное приближение даёт заметную ошибку.
...
**Стальная столешница (1.5 мм).** Сквозь столешницу к кристаллу проходит 43 % направлений — 35 % воздушного геометрического фактора и весь вклад пола; лист стали ослабляет этот нижний поток на −0.020 нЗв/ч на 100 Бк/м³ (расчёт по всем 110 линиям с наклонными толщинами, NIST-данные железа). Одновременно нижняя плоскость столешницы — поверхность осаждения в ~4.5 см от кристалла, светящая сквозь ту же сталь: +0.035…+0.045 нЗв/ч на 100 Бк/м³ (активность по потолочному прокси ориентации). Нетто-поправка **+0.013…+0.028 нЗв/ч на 100 Бк/м³** входит в базовую оценку.
...
### 4.1. Коэффициент равновесия — выход модели

При измеренной вентиляции коэффициент равновесия определяется запылённостью:

**Таблица 2. Коэффициент равновесия и свободная доля ²¹⁸Po как функции запылённости (λв = 0.02 ч⁻¹).**

| X, ч⁻¹ | Уровень запылённости | F (выход) | Свободная доля ²¹⁸Po |
|---|---|---|---|
...
Эксперимент #RN3 (выключение вентиляции, рост ОА радона 318 → 1193 Бк/м³) дал коэффициент связи МЭД с ОА **+1.09 нЗв/ч на 100 Бк/м³** (межсуточный контраст состояний, 95 % ДИ 1.03–1.16)[15]. Внутрисуточные оценки — свободные от примеси суточного хода температуры — ниже: 0.94 ± 0.34 (p = 0.013) и 0.69 ± 0.47 (незначимо), то есть реалистичный диапазон 0.7–1.1.

Помимо статистического ДИ, целевая величина несёт **калибровочную погрешность прибора: основная относительная погрешность БДКГ-05 ±20 %** (паспорт, эталон Cs-137). Это систематический множитель показания МЭД; коэффициент — наклон дозы по ОА — масштабируется им целиком, отдельно от статистики. С учётом ±20 % межсуточный контраст расширяется до полосы **0.87–1.31**, внутрисуточные оценки — до **0.55–1.13** нЗв/ч на 100 Бк/м³.
...
| Замер, межсуточный (стат. ДИ) | 1.09 (1.03–1.16) | ×1.6 к базовой |
| Замер, межсуточный + инстр. ±20 % | 0.87–1.31 | ×1.26 к нижнему краю |
| Замер, внутрисуточные + инстр. ±20 % | 0.55–1.13 | **перекрывается** (×1.00 к 0.69) |

Базовая оценка модели **перекрывается с внутрисуточной полосой замера**: центр модели 0.69 совпадает с центром внутрисуточной оценки 0.69. К межсуточному контрасту остаётся умеренный разрыв — ×1.6 к центру 1.09 и ×1.26 к нижнему краю инструментальной полосы. Физический механизм (гамма ДПР радона) подтверждён количественно.
...
3. **ДПР торона** (²¹²Pb, ²⁰⁸Tl 2.61 МэВ) — не учтены.

## 6. Ограничения

- Изотропно перемешанный воздух, стационар (переходные процессы 3–4 ч после смены вентиляции не моделируются).
...
- Скорости осаждения привязаны по типу поверхности, а не измерены для конкретных материалов; для стали подставки — стекло-прокси.
- Торон (²²⁰Rn) и его ДПР не учтены; осаждение на корпус прибора не моделируется; самопоглощение в корпусе не моделируется.
...
4. Целевая величина замера с учётом калибровочной погрешности БДКГ-05 (±20 %) образует полосы 0.87–1.31 (межсуточная) и 0.55–1.13 (внутрисуточная). Базовая оценка модели **совместима с внутрисуточным замером** (×1.00 к центру 0.69) и отстоит от нижнего края межсуточной полосы в ×1.26. Физический механизм — гамма ²¹⁴Pb/²¹⁴Bi — подтверждён количественно.
5. Проверены и исключены: рост скоростей осаждения гамма-эмиттеров (связка с балансом воздуха), нулевое осаждение, подпольная гамма (постоянный фон), угловой отклик прибора. Кандидаты умеренного остатка к межсуточному центру: осаждение на корпус прибора, энергетический отклик БДКГ-05 по спектру ДПР, ДПР торона.
...
The production term contains the decay constant of the *daughter* nuclide λᵢ: Cᵢ₋₁ — the parent activity — equals the birth rate of daughter *atoms*, and multiplying by λᵢ converts the atom balance into an activity balance.
...
**The steel tabletop (1.5 mm).** 43 % of the directions reach the crystal through the tabletop — 35 % of the air geometric factor and the entire floor contribution; the steel sheet attenuates this from-below flux by −0.020 nSv/h per 100 Bq/m³ (computed over all 110 lines with slant thicknesses, NIST iron data). At the same time the tabletop's underside is a deposition surface ~4.5 cm from the crystal shining through the same steel: +0.035…+0.045 nSv/h per 100 Bq/m³ (activity per the ceiling orientation proxy). The net correction of **+0.013…+0.028 nSv/h per 100 Bq/m³** enters the base estimate.
...
At the measured ventilation the equilibrium factor is set by the aerosol loading:

**Table 2. Equilibrium factor and unattached ²¹⁸Po fraction vs aerosol loading (λᵥ = 0.02 h⁻¹).**

| X, h⁻¹ | Dust level | F (output) | Unattached ²¹⁸Po fraction |
|---|---|---|---|
...
Besides the statistical CI, the target value carries the **calibration error of the instrument: the BDKG-05 basic relative error is ±20 %** (passport, Cs-137 reference). This is a systematic multiplier of the dose-rate reading; the coefficient — the slope of dose vs concentration — is scaled by it in full, separately from the statistics. With ±20 % included, the day-to-day contrast widens to a band of **0.87–1.31** and the within-day estimates to **0.55–1.13** nSv/h per 100 Bq/m³.
...
3. **Thoron progeny** (²¹²Pb, ²⁰⁸Tl 2.61 MeV) — not included.
...
- Deposition velocities are mapped by surface type rather than measured for the specific materials; the steel stand uses a glass proxy.
- Thoron (²²⁰Rn) and its progeny are not included; deposition on the instrument housing is not modelled; housing self-absorption is not modelled.
...
5. Examined and excluded: higher gamma-emitter deposition velocities (coupled to the air balance), zero deposition, sub-floor gamma (constant background), the instrument's angular response. Candidates for the moderate remainder to the day-to-day centre: deposition on the instrument housing, the BDKG-05 energy response over the progeny spectrum, thoron progeny.
