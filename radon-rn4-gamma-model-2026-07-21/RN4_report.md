...
**Вывод:** истинное поле — **0.59–0.64 нЗв/ч на 100 Бк/м³** (аналитика 0.59–0.64, Geant4 0.594–0.627 — согласие двух независимых методов); ожидаемое *показание* БДКГ-05 выше поля в ×1.22–1.27 за счёт осаждения ДПР на корпус прибора (11–14 % показания) и близких осаждённых источников: **0.75–0.76 нЗв/ч на 100 Бк/м³**. Показание попадает внутрь полосы замера в пределах одних суток (0.55–1.13, с инстр. ±20 %); к центру оценки по разным суткам (1.09) остаётся ×1.43–1.45. Энергетический и угловой отклик прибора по спектру ДПР искажения не вносят (свёртка 1.004); итог слабо чувствителен к запылённости и коэффициенту равновесия (сценарий F = 0.37–0.40 меняет показание на −2 %, поле на −10 %).
...
С замером корректно сравнивать не поле, а *ожидаемое показание прибора* — 0.754–0.764 нЗв/ч на 100 Бк/м³ (§5.3): коэффициент #RN3 получен по показаниям того же БДКГ-05, в которые входят и осаждение на корпус, и отклик прибора. Показание **лежит внутри полосы замера в пределах одних суток** (0.55–1.13). К оценке по разным суткам остаётся умеренный разрыв — ×1.43–1.45 к центру 1.09 и ×1.14–1.15 к нижнему краю инструментальной полосы. Физический механизм (гамма ДПР радона) подтверждён количественно двумя независимыми методами.
...
Показание модели (0.75–0.76) согласуется с оценками в пределах одних суток; разрыв ×1.43–1.45 остаётся только к оценке по разным суткам (1.09). Возможные вклады в этот остаток:
...
5. Целевая величина замера с учётом калибровочной погрешности БДКГ-05 (±20 %) образует полосы 0.87–1.31 (по разным суткам) и 0.55–1.13 (в пределах одних суток). Ожидаемое показание **лежит внутри полосы оценок в пределах одних суток**; к центру оценки по разным суткам остаётся ×1.43–1.45 (×1.14–1.15 к нижнему краю её инструментальной полосы). Физический механизм — гамма ²¹⁴Pb/²¹⁴Bi — подтверждён количественно двумя независимыми методами.
...
What should be compared with the measurement is not the field but the *expected instrument reading* — 0.754–0.764 nSv/h per 100 Bq/m³ (§5.3): the #RN3 coefficient was obtained from the readings of the same BDKG-05, which include both the deposition on the housing and the instrument response. The reading **lies inside the within-day measurement band** (0.55–1.13). A moderate gap remains to the day-to-day estimate — ×1.43–1.45 to the centre 1.09 and ×1.14–1.15 to the lower edge of the instrument band. The physical mechanism (radon progeny gamma) is confirmed quantitatively by two independent methods.
...
The model reading (0.75–0.76) agrees with the within-day estimates; the ×1.43–1.45 gap remains only to the day-to-day estimate (1.09). Possible contributions to this remainder:
...
5. The measurement target with the BDKG-05 calibration error (±20 %) forms the bands 0.87–1.31 (day-to-day) and 0.55–1.13 (within-day). The expected reading **lies inside the within-day band**; ×1.43–1.45 remains to the day-to-day centre (×1.14–1.15 to the lower edge of its instrument band). The physical mechanism — ²¹⁴Pb/²¹⁴Bi gamma — is confirmed quantitatively by two independent methods.
