...
**Вывод:** истинное поле — **0.59–0.64 нЗв/ч на 100 Бк/м³** (аналитика 0.59–0.64, Geant4 0.594–0.627 — согласие двух независимых методов); ожидаемое *показание* БДКГ-05 выше поля в ×1.22–1.27 за счёт осаждения ДПР на корпус прибора (11–14 % показания) и близких осаждённых источников: **0.75–0.76 нЗв/ч на 100 Бк/м³**. Сравнение с замером #RN3 — в тех же единицах, нЗв/ч на 100 Бк/м³: коэффициент, измеренный по данным одних суток (с приборной погрешностью ±20 %), — 0.55–1.13, и рассчитанное показание 0.75–0.76 попадает внутрь этого диапазона; коэффициент по сравнению разных суток — 1.09, он выше рассчитанного показания в 1.43–1.45 раза. Энергетический и угловой отклик прибора по спектру ДПР искажения не вносят (свёртка 1.004); итог слабо чувствителен к запылённости и коэффициенту равновесия (сценарий F = 0.37–0.40 меняет показание на −2 %, поле на −10 %).
...
| Замер, по разным суткам (стат. ДИ) | 1.09 (1.03–1.16) | ×1.43–1.45 к показанию |
| Замер, по разным суткам + инстр. ±20 % | 0.87–1.31 | ×1.14–1.15 к нижнему краю |
| Замер, в пределах одних суток + инстр. ±20 % | 0.55–1.13 | **показание внутри полосы** |

С замером корректно сравнивать не поле, а *ожидаемое показание прибора* — 0.754–0.764 нЗв/ч на 100 Бк/м³ (§5.3): коэффициент #RN3 получен по показаниям того же БДКГ-05, в которые входят и осаждение на корпус, и отклик прибора. Показание **лежит внутри диапазона оценок в пределах одних суток** (0.55–1.13 нЗв/ч на 100 Бк/м³). К оценке по разным суткам остаётся умеренный разрыв: её центр 1.09 выше рассчитанного показания в 1.43–1.45 раза, нижний край инструментального диапазона 0.87 — в 1.14–1.15 раза. Физический механизм (гамма ДПР радона) подтверждён количественно двумя независимыми методами.
...
### 6.4. Кандидаты остатка к оценке по разным суткам

Показание модели (0.75–0.76) согласуется с оценками в пределах одних суток; разрыв в 1.43–1.45 раза остаётся только к оценке по разным суткам (1.09). Возможные вклады в этот остаток:
...
2. **Вариабельность самого замера** — сигнал мал относительно фона (§6.1), и оценка по разным суткам включает межсуточные изменения фона, которых нет в оценках в пределах суток.
...
5. Измеренный коэффициент с учётом калибровочной погрешности БДКГ-05 (±20 %) лежит в диапазонах 0.87–1.31 (по разным суткам) и 0.55–1.13 (в пределах одних суток) нЗв/ч на 100 Бк/м³ — в тех же единицах, что и расчёт. Рассчитанное показание 0.75–0.76 **попадает внутрь диапазона оценок в пределах одних суток**; коэффициент по сравнению разных суток (1.09) выше рассчитанного показания в 1.43–1.45 раза (нижний край его инструментального диапазона, 0.87, — в 1.14–1.15 раза). Физический механизм — гамма ²¹⁴Pb/²¹⁴Bi — подтверждён количественно двумя независимыми методами.
6. Проверены и исключены: энергетический и угловой отклик прибора (Geant4 + паспорт), рост скоростей осаждения гамма-эмиттеров (связка с балансом воздуха), нулевое осаждение, ДПР торона (замер АльфаАэро). Кандидаты остатка к оценке по разным суткам: скорость осаждения на корпус, вариабельность замера при малом отношении сигнал/фон, коррелированная с ОА подпольная гамма и внешнее поле (§6.4).
...
Код и воспроизведение: аналитика — `scripts/analysis/rn_room_model.py` + `rn_v9_lroom.py` + `rn_v9_tables.py` + `rn_vent_from_rise.py` + `rn_tabletop_correction.py` + `rn_bdkg_band.py` (Python 3.12 + numpy); Монте-Карло — Geant4 11.2, сцена `bdkg05_scene` (C++), свёртка `postproc_v8.py`. Постоянные распада: ²¹⁸Po 13.42 ч⁻¹, ²¹⁴Pb 1.552 ч⁻¹, ²¹⁴Bi 2.111 ч⁻¹. Скорости осаждения — `references/deposition-velocities-materials.md`. Журнал изменений — `CHANGELOG_RN4_v9.md`.
...
| Measurement, day-to-day (stat. CI) | 1.09 (1.03–1.16) | ×1.43–1.45 vs reading |
| Measurement, day-to-day + instr. ±20 % | 0.87–1.31 | ×1.14–1.15 to lower edge |
| Measurement, within-day + instr. ±20 % | 0.55–1.13 | **reading inside the band** |

What should be compared with the measurement is not the field but the *expected instrument reading* — 0.754–0.764 nSv/h per 100 Bq/m³ (§5.3): the #RN3 coefficient was obtained from the readings of the same BDKG-05, which include both the deposition on the housing and the instrument response. The reading **lies inside the range of the within-day estimates** (0.55–1.13 nSv/h per 100 Bq/m³). A moderate gap remains to the day-to-day estimate: its centre, 1.09, is 1.43–1.45 times the computed reading, and the lower edge of its instrument range, 0.87, is 1.14–1.15 times it. The physical mechanism (radon progeny gamma) is confirmed quantitatively by two independent methods.
...
### 6.4. Candidates for the remainder towards the day-to-day estimate

The model reading (0.75–0.76) agrees with the within-day estimates; a 1.43–1.45-fold gap remains only to the day-to-day estimate (1.09). Possible contributions to this remainder:
...
2. **Variability of the measurement itself** — the signal is small against the background (§6.1), and the day-to-day estimate includes day-to-day background changes absent from the within-day estimates.
...
5. The measured coefficient with the BDKG-05 calibration error (±20 %) spans 0.87–1.31 (day-to-day) and 0.55–1.13 (within-day) nSv/h per 100 Bq/m³ — the same units as the calculation. The computed reading of 0.75–0.76 **falls inside the within-day range**; the day-to-day coefficient (1.09) is 1.43–1.45 times the computed reading (the lower edge of its instrument range, 0.87, is 1.14–1.15 times it). The physical mechanism — ²¹⁴Pb/²¹⁴Bi gamma — is confirmed quantitatively by two independent methods.
6. Examined and excluded: the instrument's energy and angular response (Geant4 + passport), higher gamma-emitter deposition velocities (coupled to the air balance), zero deposition, thoron progeny (AlfaAero measurement). Candidates for the remainder towards the day-to-day estimate: the deposition velocity on the housing, the variability of the measurement at a small signal-to-background ratio, sub-floor gamma correlated with the room concentration, and the external field (§6.4).
