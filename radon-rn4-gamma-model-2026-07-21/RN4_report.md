...
**Вывод:** аналитическая модель и независимый расчёт Geant4 согласованно дают поле в точке детектора **0.59–0.64 нЗв/ч на 100 Бк/м³** (аналитика 0.59–0.64; Geant4 0.594–0.627). Показание прибора превышает поле в 1.22–1.27 раза — вклад осаждения ДПР на корпус блока детектирования (11–14 % показания) и близких осаждённых источников; ожидаемое показание БДКГ-05 — **0.75–0.76 нЗв/ч на 100 Бк/м³**. В эксперименте #RN3 измерено не поле, а приращение мощности амбиентного эквивалента дозы на единицу ОА радона — коэффициент **+1.09 нЗв/ч на 100 Бк/м³** (95 % ДИ 1.03–1.16; с инструментальной погрешностью ±20 % — полоса 0.87–1.31). Он превышает расчётное показание в 1.43–1.45 раза, а его нижняя граница — в 1.14–1.15 раза, то есть расхождение выходит за пределы погрешности замера. Расчёт и измерение согласуются в пределах фактора 1.5 (к нижней границе полосы замера — в пределах 15 %); природа расхождения рассмотрена в §6.4. Инструментальный отклик его не объясняет: свёртка энергетической и угловой чувствительности по спектру ДПР даёт 1.004. Не объясняет и неизмеренная запылённость: при коэффициенте равновесия 0.37–0.40 (чистый воздух) показание смещается на −2 %, поле — на −10 %.
...
**Conclusion:** the analytical model and the independent Geant4 calculation give a consistent field at the detector location, **0.59–0.64 nSv/h per 100 Bq/m³** (analytics 0.59–0.64; Geant4 0.594–0.627). The instrument reading exceeds the field by a factor of 1.22–1.27 — the contribution of progeny deposition on the detector-unit housing (11–14 % of the reading) and of the nearby deposited sources; the expected BDKG-05 reading is **0.75–0.76 nSv/h per 100 Bq/m³**. Experiment #RN3 measured not the field but the increment of the ambient dose equivalent rate per unit radon concentration — a coefficient of **+1.09 nSv/h per 100 Bq/m³** (95 % CI 1.03–1.16; with the ±20 % instrument error, a band of 0.87–1.31). It exceeds the computed reading by a factor of 1.43–1.45, and its lower bound by 1.14–1.15, so the discrepancy lies outside the measurement error limits. The calculation and the measurement agree to within a factor of 1.5 (within 15 % of the lower bound of the measurement band); the nature of the discrepancy is examined in §6.4. The instrument response does not account for it: the convolution of the energy and angular sensitivity over the progeny spectrum is 1.004. Nor does the unmeasured dust level: at an equilibrium factor of 0.37–0.40 (clean air) the reading shifts by −2 %, the field by −10 %.
