...
- **Нулевое осаждение** — сценарий не согласуется с данными: удержание наблюдаемого неравновесия одной вентиляцией потребовало бы λв ≈ 1.4–2.6 ч⁻¹ (F = 0.45…0.30; полный воздухообмен каждые 23–43 минуты), что не согласуется с измеренной динамикой; доза при этом снизилась бы в разы.
...
The radon build-up curve of experiment #RN3[15] allows λᵥ to be determined. The radon balance in the well-mixed room air is the input from a constant source S (soil-gas entry) minus removal through two channels, ventilation and self-decay:
...
- **The BDKG-05 angular response** — the passport Table G.6 (isotropic-field convolution +3.5 % Co-60 … +5.9 % Cs-137) and Monte Carlo (−3…+2 %, §5.1) both give a few-per-cent effect and do not account for the observed discrepancy.
...
6. Examined and found insignificant: the instrument's energy and angular response (Geant4 + passport), higher gamma-emitter deposition velocities (coupled to the air balance), the zero-deposition scenario; thoron progeny are not detected in the AlfaAero measurement. Possible sources of the residual discrepancy: the deposition velocity on the housing, the variability of the measurement at a small signal-to-background ratio, sub-floor gamma correlated with the room concentration, and the external field (§6.4).
